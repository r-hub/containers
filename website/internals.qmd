...
The LAPACK in R 4.5.0 and later needs `dgemmtr`, which BLIS has from version 2.0 on.
Fedora ships BLIS 2.0 from Fedora 43, so this container uses Fedora 44 and the R-devel build of the `gcc16` container.
Its shared BLAS (`libRblas.so`) is a symlink to serial BLIS, and R keeps using its own LAPACK.
